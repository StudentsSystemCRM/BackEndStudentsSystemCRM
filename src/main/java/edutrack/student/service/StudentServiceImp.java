...
public class StudentServiceImp implements StudentService {
	...
	public List<StudentDataResponse> getAllStudents() {
		...
		return studentResponse.stream().map(EntityDtoStudentMapper.INSTANCE::studentToStudentDataResponse)
	...
	public StudentDataResponse updateStudent(StudentUpdateDataRequest student) {
		StudentEntity studentEntity = studentRepo.findById(student.getId()).orElseThrow(
				() -> new StudentNotFoundException("Student with id " + student.getId() + " doesn't exist"));
		...
		if (!student.getEmail().equals(studentEntity.getEmail())) {
			...
			if (existingStudent != null) {
				throw new EmailAlreadyInUseException(
						"Email " + student.getEmail() + " is already in use by another student.");
			}
		}
	...
	public List<StudentDataResponse> getStudentsByGroup(String name) {
		...
		return students.stream().map(EntityDtoStudentMapper.INSTANCE::studentToStudentDataResponse)
				.collect(Collectors.toList());
	}
}
