...
//        		,group.getStudents(),group.getGroupReminders()
	...
	private GroupEntity findGroupByName(String name) {
        ...
        if (groupEntity == null)
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Group with name " + name + "  not found");
        return groupEntity;
	}
...
//		 List<String> groups = groupRepo.getGroups(id);
//		 List<GroupEntity> groupResponse = groupRepo.findAllStudentGroups(groups);
		 StudentEntity student = findStudentById(id);
	...
	public GroupDataResponse deleteStudentFromGroup(Long id, String name) {
		StudentEntity student = findStudentById(id);
        GroupEntity group = findGroupByName(name);
        if (!group.getStudents().remove(findStudentById(id))) throw new StudentNotFoundException("Student with id " + id + " not found in group " + name);
        groupRepo.save(group);
