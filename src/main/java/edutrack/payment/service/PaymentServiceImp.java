...
public class PaymentServiceImp implements PaymentService {
	...
	public PaymentInfoResponse addStudentPayment(AddPaymentRequest studentPayment) {
		StudentEntity student = studentRepo.findById(studentPayment.getStudentId()).orElseThrow(
				() -> new StudentNotFoundException("Student with id " + studentPayment.getStudentId() + " not found"));
		PaymentEntity savedPayment = EntityDtoPaymentMapper.INSTANCE.addPaymentRequestToPayment(studentPayment,
				student);
		savedPayment = paymentRepo.save(savedPayment);
		return EntityDtoPaymentMapper.INSTANCE.studentToPaymentInfoResponse(student,
				getListOfSinglePaymentsById(studentPayment.getStudentId()));
	}

	private List<SinglePayment> getListOfSinglePaymentsById(Long id) {
		List<PaymentEntity> payments = paymentRepo.findByStudentId(id);
		List<SinglePayment> paymentsResp = (payments != null) ? payments.stream()
				.map(EntityDtoPaymentMapper.INSTANCE::paymentToSinglePayment).collect(Collectors.toList())
