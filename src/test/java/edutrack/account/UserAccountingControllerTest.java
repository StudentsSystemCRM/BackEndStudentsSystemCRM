...
public class UserAccountingControllerTest {
	...
	final UserRegisterRequest USER_REGISTER_REQUEST = UserRegisterRequest.builder().email(VALID_USER_EMAIL_1)
			.password(VALID_USER_PASSWORD_1).name(VALID_USER_NAME_1).surname(VALID_USER_SURNAME_1)
			.phone(VALID_USER_PHONE_1).birthdate(VALID_BIRTHDATE).build();
	final LoginSuccessResponse LOGIN_SUCCESS_RESPONSE = LoginSuccessResponse.builder().token(VALID_TOKEN)
			.name(VALID_USER_NAME_1).surname(VALID_USER_SURNAME_1).phone(VALID_USER_PHONE_1).birthdate(VALID_BIRTHDATE)
			.createdDate(VALID_CREATED_DATE).roles(VALID_ROLE).build();

	final UserUpdateRequest USER_UPDATE_REQUEST = UserUpdateRequest.builder().email(VALID_USER_EMAIL_1)
			.name(VALID_USER_NAME_1).surname(VALID_USER_SURNAME_1).phone(VALID_USER_PHONE_1).birthdate(VALID_BIRTHDATE)
			.build();

	final UserDataResponse USER_DATA_RESPONSE = UserDataResponse.builder().email(VALID_USER_EMAIL_1)
			.name(VALID_USER_NAME_1).surname(VALID_USER_SURNAME_1).phone(VALID_USER_PHONE_1).birthdate(VALID_BIRTHDATE)
			.createdDate(VALID_CREATED_DATE).roles(VALID_ROLE).build();
	...
	void testRegisterUserWhenValidUserRegisterRequestIsProvidedThenReturnLoginSuccessResponse() {
		// Arrange
		...
		// Act
		ResultActions resultActions = mockMvc.perform(post("/api/users/register").param("invite", invite)
				.contentType(MediaType.APPLICATION_JSON).content(jsonRequestData));
		// Assert
	...
	void testCreateUserWhenUserWithDuplicateIdIsProvidedThenReturnsBadRequest() {
		UserRegisterRequest requestData = USER_REGISTER_REQUEST;
		String jsonRequestData = objectMapper.writeValueAsString(requestData);
		String invite = "empty";
		...
		// Asserts
		resultActions.andExpect(status().isBadRequest()).andExpect(jsonPath("$.message").value(expectedErrorMessage));
	}

	@Test
	@SneakyThrows
	@DisplayName("Create user, invalid password")
	void testCreateUserWhenUserWithNotValidPasswordIsProvidedThenReturnBadRequest() {
		// Arrange
		UserRegisterRequest requestData = USER_REGISTER_REQUEST.withPassword("12345").withName("123")
	...
	@WithMockUser
	void testLoginWhenValidInputIsProvidedThenReturnLoginSuccessResponse() {
		// Arrange
		...
		// Act
		ResultActions resultActions = mockMvc.perform(post("/api/users/login").contentType(MediaType.APPLICATION_JSON));
	...
	void testLoginWhenValidInputIsNotProvidedThenReturnUnauthorized() {
	...
	void testUpdateWhenValidInputIsProvidedThenReturnUserDataResponse() {
		// Arrange
		...
		ResultActions resultActions = mockMvc
				.perform(put("/api/users/update").with(user("user").password("123").roles("USER"))
						.contentType(MediaType.APPLICATION_JSON).content(requestDataJson));
	...
	void testUpdateWhenInvalidInputIsProvidedThenReturnBadRequest() {
		// Arrange
		UserUpdateRequest requestData = USER_UPDATE_REQUEST.withBirthdate(LocalDate.of(2026, 1, 1))
				.withSurname("userov.surname");
		String requestDataJson = objectMapper.writeValueAsString(requestData);
		...
		// Act
		ResultActions resultActions = mockMvc
				.perform(put("/api/users/update").contentType(MediaType.APPLICATION_JSON).content(requestDataJson));
	...
	void testUpdateWhenValidInputIsProvidedThenThrowNoPermissionAndReturnBadRequest() {
		// Arrange
		UserUpdateRequest requestData = USER_UPDATE_REQUEST;
		String requestDataJson = objectMapper.writeValueAsString(requestData);
		String expectedErrorMessage1 = "You don't have rules to update this user's profile.";

		when(accountingManagementService.updateUser(any(UserUpdateRequest.class)))
				.thenThrow(new AccessException(expectedErrorMessage1));
		...
		// Assert
		resultActions.andExpect(status().isBadRequest()).andExpect(jsonPath("$.message").isString())
	...
	void testUpdateWhenValidInputIsProvidedThenThrowNoSuchElementExceptionAndReturnBadRequest() {
		// Arrange
		UserUpdateRequest requestData = USER_UPDATE_REQUEST;
		String requestDataJson = objectMapper.writeValueAsString(requestData);
		...
		when(accountingManagementService.updateUser(any(UserUpdateRequest.class)))
				.thenThrow(new NoSuchElementException(expectedErrorMessage1));
	...
	@WithMockUser
	void testUpdatePasswordwhenValidPasswordUpdateRequestIsProvidedThenReturn200() {
		// Arrange
		...
		// Act
		ResultActions resultActions = mockMvc
				.perform(put("/api/users/update-password").contentType("application/json").content(requestDataJson));
	...
	@WithMockUser
	void testUpdatePasswordWhenInvalidPasswordUpdateRequestIsProvidedThenReturnBadRequest() {
		// Arrange
		...
		verify(accountingManagementService, never()).updatePassword(any(Principal.class),
	...
	@WithMockUser
	void testUpdatePasswordWhenIncorrectUpdateRequestIsProvidedThenReturnBadRequest() {
		// Arrange
		...
		// Assert
		resultActions.andExpect(status().isBadRequest()).andExpect(jsonPath("$.message").value(
				"An unexpected error: JSON parse error: Unrecognized token 'password': was expecting (JSON String, Number, Array, Object or token 'null', 'true' or 'false')"));
		verify(accountingManagementService, never()).updatePassword(any(Principal.class),
				any(PasswordUpdateRequest.class));
	}
	...
	void testAddRoleWhenValidUserRoleRequestIsProvidedThenReturnUserDataResponse() {
		// Arrange
	...
	void testAddRoleWhenInvalidUserRoleRequestIsProvidedThenReturnBadRequest() {
		// Arrange
		...
		String expectedErrorMessage2 = UserRoleRequest.class.getDeclaredField("role").getAnnotation(ValidRole.class)
				.message();
	...
	@WithMockUser(username = VALID_USER_EMAIL_1, roles = { "ADMIN" })
	void testAddRoleWhenInvalidBodyIsProvidedThenReturnBadRequest() {
		// Arrange
		String invalidLogin = "invalid-email";
		...
		// Assert
		verify(accountingManagementService, never()).addRole(any(String.class), any(UserRoleRequest.class));
		resultActions.andExpect(status().isBadRequest()).andExpect(jsonPath("$.message").value(expectedErrorMessage1));
	...
	void testAddRoleWhenUserIsNotAdminThenReturnForbidden() {
		// Arrange
		String login = VALID_USER_EMAIL_1;
		String requestDataJson = objectMapper.writeValueAsString(USER_ROLE_REQUEST);
	...
	void testRemoveRoleWhenValidUserRoleRequestIsProvidedThenReturnUserDataResponse() {
		// Arrange
		String login = VALID_USER_EMAIL_1;
		String currentLogin = SecurityContextHolder.getContext().getAuthentication().getName();
		String requestDataJson = objectMapper.writeValueAsString(USER_ROLE_REQUEST);
	...
	@WithMockUser(username = VALID_USER_EMAIL_1, roles = { "ADMIN" })
	void testRemoveRoleWhenInvalidUserRoleRequestIsProvidedThenReturnBadRequest() {
		// Arrange
		String invalidLogin = "invalid-email";
		UserRoleRequest userRoleRequest = new UserRoleRequest("SUPER_ADMIN");
		String requestDataJson = objectMapper.writeValueAsString(userRoleRequest);
	...
	@WithMockUser(username = VALID_USER_EMAIL_1, roles = { "ADMIN" })
	void testRemoveRoleWhenInvalidBodyIsProvidedThenReturnBadRequest() {
		// Arrange
		String invalidLogin = "invalid-email";
		String requestDataJson = "user:SuperAdmin";
		String expectedErrorMessage1 = "An unexpected error: JSON parse error: Unrecognized token 'user': was expecting (JSON String, Number, Array, Object or token 'null', 'true' or 'false')";
	...
	@WithMockUser(username = "user", roles = { "USER" })
	void testRemoveRoleWhenUserIsNotAdminThenReturnForbidden() {
		// Arrange
		String validLogin = VALID_USER_EMAIL_1;

		// Act
		ResultActions resultActions = mockMvc
				.perform(delete("/api/users/remove-role/{login}", validLogin).contentType(MediaType.APPLICATION_JSON));
	...
	@WithMockUser(username = "admin", roles = { "ADMIN" })
	void testRemoveUserWhenValidLoginIsProvidedThenReturnUserDataResponse() {
		// Arrange
		String validLogin = VALID_USER_EMAIL_1;
		...
		// Act
		ResultActions resultActions = mockMvc
				.perform(delete("/api/users/{login}", validLogin).contentType(MediaType.APPLICATION_JSON));
	...
	@WithMockUser(username = "admin", roles = { "ADMIN" })
	void testRemoveUserWhenInvalidEmailIsProvidedThenReturnBadRequest() {
		// Arrange
		String invalidLogin = "invalid-email";
		...
		// Act
		ResultActions resultActions = mockMvc
				.perform(delete("/api/users/{login}", invalidLogin).contentType(MediaType.APPLICATION_JSON));
	...
	@WithMockUser(username = "admin", roles = { "ADMIN" })
	void testRemoveUserWhenUserDoesNotExistThenReturnNotFound() {
		// Arrange
		...
		// Act
		ResultActions resultActions = mockMvc
				.perform(delete("/api/users/{login}", nonExistentLogin).contentType(MediaType.APPLICATION_JSON));
