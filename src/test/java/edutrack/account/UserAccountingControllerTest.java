...
public class UserAccountingControllerTest {
	...
	void testUpdate_whenValidInputIsProvided_thenThrowNoPermissionAndReturnForbidden() {
		...
		when(accountingManagementService.updateUser(any(UserUpdateRequest.class)))
				.thenThrow(new AccessRoleException(expectedErrorMessage1));
		...
		// Assert
		resultActions.andExpect(status().isForbidden()).andExpect(jsonPath("$.message").isString())
	...
	void testUpdatePassword_whenValidPasswordUpdateRequestIsProvided_thenReturn200() {
		...
		// Act
		ResultActions resultActions = mockMvc.perform(put("/api/users/update-password").principal(mockPrincipal)
				.contentType("application/json").content(requestDataJson));
	...
	void testRemoveRole_whenUserIsNotAdmin_thenReturnBadRequest() {
		...
		ResultActions resultActions = mockMvc.perform(
				delete("/api/users/remove-role/{login}", VALID_USER_EMAIL_1).contentType(MediaType.APPLICATION_JSON));
